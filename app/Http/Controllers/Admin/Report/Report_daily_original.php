    ...
    public function datatable(Request $request)
    {
       ...
       // \DB::enablequerylog();
        $table = T_mcus::orderByDesc('updated_at')
                ...
                ->with('m_employee:id_m_employee,nm_m_employee,nip_m_employee,business_unit_m_employee,basetown_m_employee')
      ...
      //  dd(\DB::getquerylog());
    	...
    	foreach ($table as $key => $value) {
            ...
            if ($value->t_qrcode->id_m_branch){

                $location=M_branch::where('id_m_branch', $value->t_qrcode->id_m_branch)->first();
    		...
    		$datas[$key][] = $i++;
    		$datas[$key][] = ($value->t_qrcode->id_m_branch_pelaksana) ? \app\Models\M_branch::where('id_m_branch',$value->t_qrcode->id_m_branch_pelaksana)->first()->nm_m_branch : null;
    		$datas[$key][] = $value->m_employee->business_unit_m_employee." - ".$value->m_employee->basetown_m_employee;
        ...
        $table = T_mcus::orderByDesc('updated_at')
               ...
               // dd(\DB::getquerylog());
                ...
                foreach($table as $value){
                    ...
                    $sheet->setCellValue('B' . $rows, $value->m_employee->nip_m_employee);
                    $sheet->setCellValue('C' . $rows, $value->m_employee->nipk_m_employee);
                    $sheet->setCellValue('D' . $rows, null);
                    $sheet->setCellValue('E' . $rows, $value->m_employee->cost_center_m_employee);
                    $sheet->setCellValue('F' . $rows, $value->m_employee->nm_m_employee);
                    $sheet->setCellValue('G' . $rows, $value->m_employee->position_m_employee);
                    $sheet->setCellValue('H' . $rows, $value->m_employee->group_m_employee);
                    $sheet->setCellValue('I' . $rows, $value->m_employee->department_m_employee);
                    $sheet->setCellValue('J' . $rows, $value->m_employee->basetown_m_employee);
                    $concat_grup_mcu = '';
                    if($value->t_mcu_det){
                        foreach($value->t_mcu_det as $det){
                            $concat_grup_mcu .= $det->m_group_mcu->nm_m_group_mcu.', ';
                        }
                    }
                    $sheet->setCellValue('K' . $rows, $concat_grup_mcu);
                    ...
                    $sheet->setCellValue('N' . $rows, $value->t_qrcode->tipe_t_qrcode);
                    $sheet->setCellValue('N' . $rows, $value->t_qrcode->tipe_t_qrcode);
                    $sheet->setCellValue('N' . $rows, $value->t_qrcode->tipe_t_qrcode);
                    $sheet->setCellValue('O' . $rows, ($value->t_qrcode->id_m_branch_pelaksana) ? \app\Models\M_branch::where('id_m_branch',$value->t_qrcode->id_m_branch_pelaksana)->first()->nm_m_branch : null);
