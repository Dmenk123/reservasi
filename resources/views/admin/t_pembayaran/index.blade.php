...
$(document).ready( function () {
    ...
    $('#datatable').on('click', '.detail', function(){
        ...
        $.ajax({
            ...
            data:{id_t_pembayaran:id_t_pembayaran},
    ...
    $('#datatable').on('click', '.transaksi-selesai', function(){
        var id_t_pembayaran = $(this).data("id_t_pembayaran");
        swal.fire({
            title: "Confirmation",
            text: 'Yakin Selesaikan Transaksi ini ?',
            icon: "warning",
            showCancelButton: !0,
            ...
            if(e.value){
                $.ajax({
                    url:"{{ route('admin.t_pembayaran.transaksi_selesai') }}",
                    method:"post",
                    headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                    data:{id_t_pembayaran:id_t_pembayaran},
                    success:function(data)
