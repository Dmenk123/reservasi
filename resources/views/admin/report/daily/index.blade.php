                    ...
                    <div class="card">
                      ...
                      <div class="card-datatable p-1">
                            ...
                            <div class="row">
                                ...
                                <label class="col-form-label" for="id_m_company">Project Name</label>

                                <div class="col-md-4 mt-1">
                                    <select class="select2 form-select" id="id_m_project" name="id_m_project">
                                        <option value="" >Choose Project</option>
                                        @foreach($id_m_project as $item_id_m_project)
                                        <option value="{{$item_id_m_project->id_m_project}}" >{{$item_id_m_project->nm_m_project}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                                ...
                                <div class="col-md-4" data-bs-toggle="tooltip" id="div_id_m_branch_pelaksana" data-bs-placement="top" title data-bs-original-title="Show data based on selected branch">
                                    <label for="">Choose PIC Branch :</label>
                                    <select  class="form-control select2" name="id_m_branch_pelaksana" id="id_m_branch_pelaksana">
                                        <option value="">All Branch</option>
                                        @foreach($id_m_branch as $item_m_branch)
                                                <option value="{{$item_m_branch->id_m_branch}}">{{$item_m_branch->nm_m_branch}}</option>
                                                @endforeach
...
table = $('#datatable').DataTable({
    ...
    pageLength: 20,
    ajax: {
        url: '{{ route('admin.report.daily.datatable') }}',
        method: 'post',
        data: function(q){
            q.start1 = start1,
            q.start2 = start2,
...
$(document).ready( function () {
    $('#open_modal_pdf').click(function(){
        ...
        $.ajax({
            url:'{!!route('admin.report.daily.iframe')!!}',
            method:'get',
...
$('#id_m_project').change(function(){
    id_m_project = $('#id_m_project').val();
    $('#id_m_company').html('');
    $.ajax({
        url:'{!!route('admin.report.daily.load_company_by_project')!!}',
        method:'post',
        dataType:'json',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data:{
...
$('#export_xls').click(function(){
    $.ajax({
        url:'{!!route('admin.report.daily.download_xls')!!}',
        method:'post',
        dataType:'json',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data:{
            ...
            start2:start2,
            id_m_branch:id_m_branch,
            id_m_branch_pelaksana:id_m_branch_pelaksana,
