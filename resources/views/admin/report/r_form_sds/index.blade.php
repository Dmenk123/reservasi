        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header border-bottom">
                    ...
                    <a href="{{route('admin.report.form_sds.download_xls',['menu'=>request()->get('menu')])}}"
    ...
    $.ajaxSetup({
        ...
        $(document).ready(function() {
            $('#datatable').DataTable({
                ...
                pageLength: 20,
                ajax: {
                    url: '{{ route('admin.report.form_sds.datatable',['menu'=>request()->get('menu')]) }}',
                    method: 'POST'
                }
            });
        });
...
$('#datatable').on('click', '.view_ans', function(){
    ...
    $('#myModalLabel1').html('');
    var id_t_sds = $(this).data("id_t_sds");
    // console.log(id_m_mcu_category);
    $('#modal_edit .modal-body').html('');
    $.ajax({
        url:"{{ route('admin.report.form_sds.view_ans',['menu' => request()->get('menu')]) }}",
        method:"POST",
...
$('#datatable').on('click', '.view_info', function(){
    ...
    $('#modal_edit .modal-body').html('');
    $.ajax({
        url:"{{ route('admin.report.form_sds.view_info',['menu' => request()->get('menu')]) }}",
        method:"POST",
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data:{id_t_sds:id_t_sds},
        success:function(data)
...
//     $.ajax({
//         url:"{{ route('admin.report.form_sds.download_xls',['menu' => request()->get('menu')]) }}",
//         method:"POST",
//         headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
//         // data:{id_t_sds:id_t_sds},
//         success:function(data)
