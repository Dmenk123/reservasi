...
<div class="alert alert-info">
    ...
    <h4 class="text-center">{{($old->id_m_branch) ? $old->m_branch->entity->nm_m_entity : ''}}<br>{{$old->m_branch->nm_m_branch}}</h4>
    @else
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-1 row">
                            ...
                            <div class="col-md-12 my-1">
                                ...
                                <input type="hidden" readonly id="id_m_employee_modal" class="form-control form-control square" name="id_m_employee_modal">
                            </div>
                        </div>
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <p><strong>Pilih setidaknya 1 (satu) jenis pemeriksaan yang telah dilakukan untuk melanjutkan Check Out</strong>
                            ...
                            <div id="virtual_dom_ae_selak_ngantuk"></div>
...
<script>
    ...
    var push_group_mcu;
    var tombol_lengkap = 'ya';
    $(document).ready( function () {
        console.log(tombol_lengkap);
        {{-- OPSI 1 : USE FORM --}}
                ...
                success:function(data)
                {
                    if($.isEmptyObject(data.error)){
                        if(data.status == true){
                            ...
                            $('#modal_checkin #form_checkin_modal #id_m_employee_modal').val(data.data[4]);
                            $('#modal_checkin #form_checkin_modal #hp').val(data.data[5]);
                            $('#modal_checkin #form_checkin_modal #email_m_employee').val(data.data[6]);

                            $('#virtual_dom_ae_selak_ngantuk').html(data.mcu_category);
                            $('#virtual_dom_ae_selak_ngantuk .pemeriksaan').not(this).prop('checked', this.checked).attr('readonly',true);
                        }else{
                            displayWarningSwal(data.message);
                            $("#btnlogin2").removeAttr('disabled');
                            $("#btnlogin2 span").text('Check ID');
    ...
    $('#btnradio1').click(function(){
        ...
        $('#virtual_dom_ae_selak_ngantuk .pemeriksaan').not(this).prop('checked', this.checked).attr('readonly',true); //MODE SAKLEK
    })

    // UNCHECK ALL PEMERIKSAAN KETIKA DI KLIK TOMBOL BELUM LENGKAP
    $('#btnradio2').click(function(){
        tombol_lengkap = 'tidak';
        $('#virtual_dom_ae_selak_ngantuk .pemeriksaan').not(this).removeAttr('readonly');
        // $('#virtual_dom_ae_selak_ngantuk .pemeriksaan').not(this).prop('checked', false).removeAttr('readonly');
    })


    // SILAHKAN KOMEN BLOK CODE INI JIKA INGIN FLEKSIBEL [START]
    $('#virtual_dom_ae_selak_ngantuk').on('click', '.pemeriksaan', function(){
