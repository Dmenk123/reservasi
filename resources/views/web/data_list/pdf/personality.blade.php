...
$t_test=\App\Models\T_test::where('id_t_applicant',request()->get('id_t_applicant'))->first();
@endphp
...
//\DB::enablequerylog();
            $personality=App\Models\M_quiz::where('id_m_test_pack', request()->get('id_m_test_pack'))->get();
             $no=1;
            // dd(\DB::getquerylog());
        @endphp

        @foreach($personality as $item_personality)


        @php
          //  \DB::enablequerylog();
            $answer_personality=App\Models\T_test_answer::with('m_quiz')->where('id_m_quiz',$item_personality->id_m_quiz)->where('id_t_test', request()->get('id_t_test'))->first();
           // dd(\DB::getquerylog());
        @endphp
        ...
        <tr>
            <td>{{$no++}}</td>
            <td>
            {!! ($item_personality->nm_m_quiz) !!}

            </td>
            <td allign="center">

            @if  ($answer_personality)

            @if ($answer_personality->ans_t_test_answer=='A')
            {{$answer_personality->m_quiz->ans_a_m_quiz}}
            @elseif ($answer_personality->ans_t_test_answer=='B')
            {{$answer_personality->m_quiz->ans_b_m_quiz}}
            @elseif ($answer_personality->ans_t_test_answer=='C')
            {{$answer_personality->m_quiz->ans_c_m_quiz}}
            @elseif ($answer_personality->ans_t_test_answer=='D')
            {{$answer_personality->m_quiz->ans_d_m_quiz}}
            @elseif ($answer_personality->ans_t_test_answer=='E')
            {{$answer_personality->m_quiz->ans_e_m_quiz}}
            @endif
